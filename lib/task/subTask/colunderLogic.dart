...
class DateUtils {
  ...
  static String formatDayWithDayName(DateTime d) {
    return DateFormat('''E 
 dd''',).format(d);
  }
  

  static List<String> formatDaysWithDayNames(List<DateTime> dates) {
    return dates.map((date) => formatDayWithDayName(date)).toList();
  }

  static List<String> daysWithDayNamesInMonth(DateTime month) {
    final List<DateTime> dates = daysInMonth(month);
    return formatDaysWithDayNames(dates);
  }
}
